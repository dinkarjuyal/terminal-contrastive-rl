...
def _zero_reward(**kwargs) -> float:
    """Verifier-free: reward comes from TC loss, not here."""
    return 0.0


def make_env(exclude_task_ids=None) -> LocalBashEnv:
    dataset = make_dataset(exclude_ids=exclude_task_ids)
    rubric = vf.Rubric(funcs=[_zero_reward], weights=[1.0])
    env = LocalBashEnv(
        dataset=dataset,
        rubric=rubric,
        max_turns=4,
...
def train(config_path: str = "configs/rl/bash_agent_tc.toml"):
    ...
    env = make_env(exclude_task_ids=exclude_task_ids or None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/rl/bash_agent_tc.toml",
                        help="Path to TOML config for training")
