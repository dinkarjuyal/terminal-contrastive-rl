...
def process_chat_format_vllm(
    ...
    _ach = processing_class.apply_chat_template(
        conversation=prompt,  # type: ignore
        add_generation_prompt=True,
    )
    prompt_ids: list[int] = list(_ach.input_ids) if hasattr(_ach, "input_ids") else list(_ach)
    ...
    while i < len(zipped):
        ...
        # user/tool case -- use message
            ...
            _tp = processing_class.apply_chat_template(
                conversation=messages_consumed,
                add_generation_prompt=False,
            )
            token_prefix: list[int] = list(_tp.input_ids) if hasattr(_tp, "input_ids") else list(_tp)
            ...
            _tpwt = processing_class.apply_chat_template(
                conversation=messages_consumed + consecutive_messages,
                add_generation_prompt=True,
            )
            token_prefix_with_turn: list[int] = list(_tpwt.input_ids) if hasattr(_tpwt, "input_ids") else list(_tpwt)
...
def process_completion_format_vllm(
    ...
    _enc = processing_class.encode(prompt)
    prompt_ids: list[int] = list(_enc.input_ids) if hasattr(_enc, "input_ids") else list(_enc)
    ...
    while i < len(zipped):
        ...
        # non-model-generated (user/tool case) -- use text
            ...
            _enc2 = processing_class.encode(rollout_consumed)
            token_prefix: list[int] = list(_enc2.input_ids) if hasattr(_enc2, "input_ids") else list(_enc2)
            _enc3 = processing_class.encode(rollout_consumed + text)
            token_prefix_with_turn: list[int] = list(_enc3.input_ids) if hasattr(_enc3, "input_ids") else list(_enc3)
            assert token_prefix_with_turn[: len(token_prefix)] == token_prefix, (
                f"Token prefix mismatch. Token prefix: {token_prefix}, token prefix with turn: {token_prefix_with_turn}"
            )
            completion_turn_ids = token_prefix_with_turn[len(token_prefix) :]
