...
def get_model(
    ...
    loader = AutoLigerKernelForCausalLM if use_liger else AutoModelForCausalLM
    def _load(**kw):
        try:
            model = loader.from_pretrained(model_name, **kw)
        except TypeError as e:
            if "use_cache" in str(e):
                kw2 = {k: v for k, v in kw.items() if k != "use_cache"}
                model = loader.from_pretrained(model_name, **kw2)
            else:
                raise
        # PEFT LoRA requires PreTrainedModel.warnings_issued; patch if missing
        if not hasattr(model, "warnings_issued"):
            model.warnings_issued = {}
        return model
    return _load(**model_kwargs)
